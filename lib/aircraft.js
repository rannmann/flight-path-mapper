...
const seattleCoord = { lat: 47.6097, lon: -122.3331 };
...
/**
 * Calculate the loudness of a plane.
 * @param {object} plane
...
/**
 * Get the size factor for a plane from the airplane database.
 *
 * @param plane
 * @returns {number|null}
 */
...
function addLoudnessToGrid(plane, grid) {
    ...
    let loudnessRadius = calculateSoundRadius(plane);
    ...
    let gridPos = findContainingGridSquare(plane);
...
function findContainingGridSquare(plane) {
    const earthCircumference = 40075; // Earth's circumference at equator in km
    const latitudeDistPerDeg = earthCircumference / 360;
    const longitudeDistPerDeg = Math.cos(degreesToRadians(seattleCoord.lat)) * earthCircumference / 360;  // computed at Seattle's latitude

    let xIndex = Math.floor((plane.lon - seattleCoord.lon) * longitudeDistPerDeg);
    let yIndex = Math.floor((plane.lat - seattleCoord.lat) * latitudeDistPerDeg);

    // grid central coordinates for each 1km square grid
    let centralLon = seattleCoord.lon + (xIndex + 0.5) / longitudeDistPerDeg;
    let centralLat = seattleCoord.lat + (yIndex + 0.5) / latitudeDistPerDeg;
