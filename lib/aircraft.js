...
function detectFlightPhase(altitude, geom_rate, speed) {
    const altFt = altitude === 'ground' ? 0 : (altitude || 0);
    const climbRate = geom_rate || 0;
    const groundSpeed = speed || 0;
    
    // Ground operations (new enhanced detection)
    if (altFt <= 50 || altitude === 'ground') {
        if (groundSpeed <= 25) {
            return 'taxi'; // Slow ground movement (taxi, parking)
        } else if (groundSpeed > 80 || climbRate > 500) {
            return 'takeoff_roll'; // Accelerating for takeoff
        } else {
            return 'ground_idle'; // Stationary or very slow movement
        }
    }
    
    // Takeoff: low altitude, high climb rate, moderate speed (airborne)
...
function estimateThrust(flightPhase, geom_rate, speed, category) {
    ...
    switch (flightPhase) {
        case 'taxi':
            return 0.10; // Low thrust for taxi operations
        case 'ground_idle':
            return 0.05; // Minimal thrust for parking/idle
        case 'takeoff_roll':
            return 1.0; // Maximum thrust during takeoff acceleration
        case 'takeoff':
            return 1.0; // Maximum thrust (airborne)
        case 'climb':
...
function findContainingGridSquare(plane, middleLat, middleLon) {
    ...
    const latitudeDistPerDeg = earthCircumference / 360; // ~111.32 km per degree latitude
    
    // For 1km grid cells, we need degrees per km
    const degreesLatPerKm = 1 / latitudeDistPerDeg; // ~0.00899 degrees per km
    
    // Calculate the y-index (latitude direction) - this is constant
    const latDiff = plane.lat - middleLat;
    const yIndex = Math.floor(latDiff / degreesLatPerKm);
    
    // Calculate the actual latitude of this grid cell's center
    const gridCellLat = middleLat + (yIndex + 0.5) * degreesLatPerKm;
    
    // Now calculate longitude compression for THIS specific grid cell's latitude
    const longitudeDistPerDeg = Math.cos(degreesToRadians(gridCellLat)) * earthCircumference / 360;
    const degreesLonPerKm = 1 / longitudeDistPerDeg;
    
    // Calculate the x-index using the proper longitude compression
    const lonDiff = plane.lon - middleLon;
    const xIndex = Math.floor(lonDiff / degreesLonPerKm);
    
    // Calculate the central coordinates of this grid cell
    const centralLon = middleLon + (xIndex + 0.5) * degreesLonPerKm;
    const centralLat = gridCellLat; // Already calculated above
...
function addLoudnessToGridOriginal(plane, grid, middleLat, middleLon) {
    ...
    // Debug mode for specific grid cells (optional)
    const DEBUG_GRID_CELL = process.env.DEBUG_GRID_CELL;
    ...
    // Debug logging for specific grid cell
    const isDebugCell = DEBUG_GRID_CELL && key === DEBUG_GRID_CELL;
    ...
    if (receivedNoiseLevel <= 30) {
        if (isDebugCell) {
            console.log(`🔇 DEBUG ${key}: Aircraft below threshold - ${receivedNoiseLevel.toFixed(1)} dB`);
        }
        return; // Below background noise threshold
    }
    
    // Debug output for target grid cell
    if (isDebugCell) {
        const flightPhase = detectFlightPhase(plane.alt_baro, plane.geom_rate, plane.gs);
        const thrustSetting = estimateThrust(flightPhase, plane.geom_rate, plane.gs, plane.category);
        
        console.log(`🛩️ DEBUG ${key}: Aircraft operation detected`);
        console.log(`  📍 Position: [${plane.lat.toFixed(6)}, ${plane.lon.toFixed(6)}]`);
        console.log(`  ✈️ Aircraft: cat=${plane.category || 'unknown'}, alt=${plane.alt_baro || 'unknown'}ft, speed=${plane.gs || 'unknown'}kts, climb=${plane.geom_rate || 'unknown'}ft/min`);
        console.log(`  🛫 Flight phase: ${flightPhase}, thrust: ${(thrustSetting * 100).toFixed(0)}%`);
        console.log(`  📏 Distance: ${distanceKm.toFixed(3)}km horizontal, ${altitudeKm.toFixed(3)}km altitude, ${slantDistKm.toFixed(3)}km slant`);
        console.log(`  🔊 Noise: ${sourceNoiseLevel.toFixed(1)} dB source → ${receivedNoiseLevel.toFixed(1)} dB received`);
        console.log(`  📊 Linear: ${Math.pow(10, receivedNoiseLevel / 10).toFixed(0)}`);
        if (plane.hex) console.log(`  🏷️ Hex: ${plane.hex}`);
    }
    ...
    if (grid[key]) {
        ...
        const oldLinearSum = grid[key].linearSum;
        const oldCount = grid[key].count;
        const oldAverage = oldLinearSum / oldCount;
        ...
        const newAverage = grid[key].linearSum / grid[key].count;
        
        if (isDebugCell) {
            console.log(`  📈 Accumulation: count ${oldCount}→${grid[key].count}, avg ${oldAverage.toFixed(0)}→${newAverage.toFixed(0)} linear (${(10*Math.log10(newAverage)).toFixed(1)} dB)`);
            console.log(`  📊 Running totals: linearSum=${grid[key].linearSum.toFixed(0)}, count=${grid[key].count}`);
        }
    } else {
        grid[key] = {
            linearSum: newLinear,
        ...
        if (isDebugCell) {
            console.log(`  🆕 First aircraft in cell: linearSum=${newLinear.toFixed(0)}, count=1`);
        }
    }
